...
public class CubeRaycastController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (Physics.Raycast(transform.position-new Vector3(GetComponent<BoxCollider>().size.x/2,0,0), direction, out hit, 1f) || 
            Physics.Raycast(transform.position + new Vector3(GetComponent<BoxCollider>().size.x/2,0,0), direction, out hit, 1f))
        {
            if (!isStack)
            {
                isStack = !isStack;
